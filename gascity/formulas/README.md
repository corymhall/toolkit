...
- two-session planned delivery (`spec -> enrich || plans -> execution beads -> execution convoy`)
...
- execution convoy — execution tracking artifact for `delivery-workflow-planned`
...
**Steps:**
1. Validate `spec.md`, `plans.md`, and execution convoy context
2. Design execution bead graph from milestone plan
3. Create/reuse execution beads under the execution convoy
...
**Input:** `docs/plans/{feature}/spec.md`, `docs/plans/{feature}/plans.md`, execution convoy from `session-context.md`
**Output:** Execution-convoy child beads with dependency graph
...
No polecat delegation. Keeps lightweight visibility through the execution
convoy and an optional tracking epic.
...
This workflow now compiles as graph.v2. Work the ready beads directly and use
the execution convoy as the primary tracking surface for the one-session run.
...
move into a convoy-owned bead graph instead of staying inside the formula.
This is now the canonical graph.v2 version of the planned workflow.
...
2. Run `gt prime`.
3. Check for the next ready workflow bead on your hook.
4. Read that bead directly and execute it.
5. Continue through the ready graph beads until the planned workflow finishes.

This is an operational boundary, not a special workflow pause primitive. The
resume behavior now relies on the graph-first ready-bead flow rather than on
`bd mol current`.

When the workflow completes, execution no longer lives in the formula. The next
step is to use `$epic-delivery` to work the execution convoy and land the owned
convoy when all tracked execution beads are complete.
